...
public class ItemPlacer : MonoBehaviour
{
    ...
    public ItemLevelData[] GetGoals()
    {
        ...
        foreach (ItemLevelData itemData in itemDatas)
        {
            if (itemData.isGoal)
            {
                goals.Add(itemData);
    ...
    private void GenerateItems()
    {
        ...
        // Generate random seed and save it to the field
        ...
        foreach (ItemLevelData itemData in itemDatas)
        {
            for (int i = 0; i < itemData.amount; i++)
            {
                ...
                Item itemInstance = PrefabUtility.InstantiatePrefab(itemData.itemPrefab, transform) as Item;
    ...
    [Button]
    private void GenerateItemsWithSeed()
    {
        while (transform.childCount > 0)
        {
            Transform t = transform.GetChild(0);
